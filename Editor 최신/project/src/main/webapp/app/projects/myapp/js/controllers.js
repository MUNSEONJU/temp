angular.module('ngEditor.controller', []).controller('MainCtrl', function($scope, $window, $document, $http, $sce) {
  ...
  // 저장한 값들 전역으로 관리
	...
	$scope.closeMsg = '※ 작업중인 데이터가 사라집니다.';
	
	$window.onbeforeunload = confirmExit;
	function confirmExit()
	{
	  return $scope.closeMsg;
	}
	  ...
	  if(urlhash == null) {
       ...
       .success(function(result) {
        ...
        if(result.status == 'success') {
          ...
          $scope.closeMsg = '#' + urlhash + ' 는 존재하지 않는 데이터 입니다.';
...
//      idoc.getElementsByTagName('html')[0].innerHTML);
  ...
  var http = ['$http', '$q'];
  var module = ['module', 'method'];
  var service = ['service1', 'service2'];
  
  $scope.pager = function(parameter){
    switch(parameter) {
    case 'boot' :
      $scope.modalTitle = '시작하기';
      $scope.modalFooter = null;
      break;    
    case 'bind' :
      $scope.modalTitle = '데이터 바인딩';
      $scope.modalFooter = null;
      break;  
    case 'module' :
      $scope.modalTitle = '모듈';
      $scope.modalFooter = module;
      break;    
    case 'directive' :
      $scope.modalTitle = '지시자';
      $scope.modalFooter = null;
      break;    
    case 'service1' :
      $scope.modalTitle = '서비스';
      $scope.modalFooter = service;
      break;    
    case 'filter' :
      $scope.modalTitle = '필터';
      $scope.modalFooter = null;
  ...
  $scope.contentMove = function() {
    if($('#content').hasClass('fullContent')) {
